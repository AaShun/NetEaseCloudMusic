...
Component({
  ...
  methods: {
    ...
    getDetail(keywords, limit, offset) {
      app.globalData.fly.get(`/search?keywords=${this.properties.value}&type=1009&limit=${limit}&offset=${offset}`).then(res => {
        this.setData({
          keysList: res.data.result.djRadios.concat(this.data.keysList)
        })
